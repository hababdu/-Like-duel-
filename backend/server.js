...
app.get('/admin/login', (req, res) => {
  try {
    ...
    console.log(`[Admin Login] Token received: ${token}, Expected: ${expectedToken}`);
    
    // Token tekshirish - istalgan token qabul qilish
    if (token) {
      // Token bor bo'lsa, admin panel'ni ko'rsatish
      res.send(`
        <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            <title>Admin Panel - Tosh-Qaychi-Qog'oz</title>
        ...
        <body>
            <div class="login-container">
                <div class="logo">
                    ...
                    <p class="text-muted">Server Admin Panel</p>
                </div>
                
                <div class="alert alert-success">
                    <h5>✅ Access Granted</h5>
                    <p>Admin token accepted. Welcome to the server control panel.</p>
                </div>
                ...
                <div class="server-info">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <small class="text-muted">Server URL:</small><br>
                            <strong>${req.headers.host}</strong>
                        </div>
                        <div class="col-6">
                    ...
                    <div class="row mt-2">
                        <div class="col-6">
                            ...
                            <strong>${token.substring(0, 15)}...</strong>
                        </div>
                        <div class="col-6">
                            <small class="text-muted">IP:</small><br>
                            <strong>${req.ip}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 text-center">
                    <button class="btn btn-custom w-100 mb-3" onclick="openAdminPanel()">
                        <i class="bi bi-speedometer2"></i> Full Admin Panel'ga o'tish
                    </button>
                    
                    <div class="d-flex gap-2">
                        <button class="btn btn-outline-primary flex-fill" onclick="testAPI()">
                            Test API
                        </button>
                        <button class="btn btn-outline-success flex-fill" onclick="refreshStats()">
                            Refresh Stats
                        </button>
                        <button class="btn btn-outline-danger flex-fill" onclick="logout()">
                            Logout
                        </button>
                    </div>
                </div>
                ...
                <div class="mt-4 text-center">
                    <small class="text-muted">
                        Server Version 2.0.0 • Powered by Node.js
                    </small>
                </div>
            </div>
            ...
            <script>
                ...
                async function loadStats() {
                    try {
                        ...
                        document.getElementById('onlinePlayers').textContent = 'Error';
                        document.getElementById('activeGames').textContent = 'Error';
                    }
                }
                
                function openAdminPanel() {
                    // Bu yerda to'liq admin panel URL'iga o'tish
                    window.location.href = \`\${API_BASE_URL}/admin?token=\${encodeURIComponent(USER_TOKEN)}\`;
                }
                ...
                function testAPI() {
                    fetch('/health')
                        .then(res => res.json())
                        .then(data => {
                            alert('Server Status: ' + data.status + '\\nUptime: ' + data.server.uptime.toFixed(0) + 's');
                        })
                        .catch(err => {
      ...
      // Token yo'q bo'lsa, login formasi
      res.send(`
        <!DOCTYPE html>
        <html>
        <head>
    ...
    res.status(500).send(`
      ...
      <body>
        <div class="error">⚠️ Server Error</div>
        <p>An internal server error occurred.</p>
        <div class="details">
          ${error.message}<br>
          Please try again or contact administrator.
        </div>
        <button onclick="window.location.href='/admin/login'">Go Back</button>
      </body>
      </html>
    `);
  }
...
app.get('/admin', (req, res) => {
  try {
    ...
    // Har qanday tokenni qabul qilish
    if (token) {
      // To'liq admin panel HTML
      res.send(`
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <div class="row">
              ...
              <div class="col-md-3 col-lg-2 sidebar">
                ...
                <nav class="nav flex-column">
                  ...
                  <a class="nav-link" href="#">
                    <i class="bi bi-trophy me-2"></i> Tournaments
                  </a>
                  <a class="nav-link" href="#">
                ...
                <div class="mt-5 pt-5 border-top">
                  <small class="text-muted">Token: ${token.substring(0, 10)}...</small>
                  <br>
                  <button class="btn btn-sm btn-outline-light w-100 mt-2" onclick="window.location.href='/admin/login'">
              ...
              <div class="col-md-9 col-lg-10 main-content">
                <h1>Dashboard</h1>
                <p class="text-muted">Welcome to Tosh-Qaychi-Qog'oz Server Admin Panel</p>
                ...
                <div class="row mt-4">
                  ...
                  <div class="col-md-3">
                    <div class="stat-card">
                      <h5>Active Games</h5>
                      <h2 id="gamesCount">0</h2>
                    </div>
                  </div>
                  <div class="col-md-3">
                    <div class="stat-card">
                      <h5>In Queue</h5>
                      <h2 id="queueCount">0</h2>
                    </div>
                  </div>
                  <div class="col-md-3">
                    <div class="stat-card">
                      <h5>Tournaments</h5>
                      <h2 id="tournamentCount">0</h2>
                    </div>
                  </div>
                </div>
                ...
                <div class="row mt-4">
                  <div class="col-12">
                    <div class="stat-card">
                      ...
                      <button class="btn btn-primary mt-3" onclick="refreshAll()">
          ...
          <script>
            ...
            async function loadStats() {
              try {
                ...
                if (data.success) {
                  document.getElementById('onlineCount').textContent = data.stats.onlineNow;
                  document.getElementById('gamesCount').textContent = data.stats.activeGames;
                  document.getElementById('queueCount').textContent = data.stats.inQueue;
                  document.getElementById('tournamentCount').textContent = data.stats.activeTournaments;
                  
                  document.getElementById('serverStatus').innerHTML = \`
                    <div class="alert alert-success">
                      <strong>✅ Server Operational</strong><br>
                      Uptime: \${Math.floor(data.stats.server?.uptime || 0)}s<br>
                      Memory: \${data.stats.memory || 'Normal'}
                    </div>
                  \`;
                }
              } catch (error) {
