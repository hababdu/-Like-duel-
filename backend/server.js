...
setInterval(() => {
  const now = Date.now();
 // ... (oldingi qismdan davom – setInterval ichidagi cleanup tugallanmoqda)
...
server.listen(PORT, () => {
  ...
  // Har 60 soniyada monitoring
  setInterval(() => {
    const memory = process.memoryUsage();
    console.log(`
  ┌───────────────────────────────────────────────────┐
  │ Real-time Stats ${new Date().toLocaleTimeString()} │
  ├───────────────────────────────────────────────────┤
  │ Online Players: ${playerSessions.size} │
  │ Active Games: ${activeGames.size} │
  │ In Queue: ${matchmakingQueue.size} │
...
app.get('/api/stats', async (req, res) => {
  try {
    const stats = {
      ...
      onlineUsers: playerSessions.size,
...
// Admin routes (token bilan himoyalangan)
...
app.get('/admin/sessions', (req, res) => {
  res.json({
    sessions: Array.from(playerSessions.entries()).map(([id, s]) => ({
      userId: id,
      status: s.status,
...
// ==================== SERVER FULLY READY ====================
console.log('🚀 Server to‘liq ishga tushdi va tayyor!');