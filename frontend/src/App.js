...
// Tanlovlar uchun ma'lumotlar (o'zgarmas)
const CHOICES = {
...
class RPSBot {
  ...
  getMostFrequent() {
    if (!this.stats || typeof this.stats !== 'object') return null;
...
function App() {
  ...
  const handleWsMessage = (data) => {
    switch (data.type) {
      case 'joined_queue':
        setNotification({ text: 'Navbatga qo‘shildingiz...', type: 'info' });
        break;
      ...
      case 'match_found':
        ...
        startTimer();
        setNotification({ 
          text: `${data.opponent?.firstName || data.opponent?.username || 'Raqib'} topildi!`, 
          type: 'success' 
        });
        break;
      ...
      case 'game_result':
        ...
        const msg = data.result === 'draw' ? 'Durang' :
                    isWin ? 'G‘alaba!' : 'Mag‘lubiyat';
        const type = data.result === 'draw' ? 'warning' :
                     isWin ? 'success' : 'error';
      ...
      case 'error':
        setNotification({ text: data.message || 'Xato yuz berdi', type: 'error' });
        break;
  ...
  const handleBotMove = (choice) => {
    ...
    const change = res === 'win' ? (difficulty === 'easy' ? 50 : difficulty === 'medium' ? 75 : 110) :
                  res === 'draw' ? 20 : -10;
  ...
  // Tanlov tugmalari uchun umumiy komponent
        ...
        <main className="game-screen">
          ...
          <div className="versus-container">
            <div className="player-side">
              ...
              <div className="choice-display">{playerChoice ? CHOICES[playerChoice]?.emoji : '?'}</div>
            </div>
            <div className="vs">VS</div>
            <div className="player-side">
              <div className="label">BOT</div>
              <div className="choice-display">{botChoice ? CHOICES[botChoice]?.emoji : '🤔'}</div>
            </div>
          </div>

          {!playerChoice && botResult === null && (
            <ChoiceButtons onChoice={handleBotMove} />
          )}
            ...
            <div className={`result-overlay ${botResult}`}>
              <h2>
                {botResult === 'win' ? 'G‘ALABA!' :
                 botResult === 'lose' ? 'MAG‘LUBIYAT' : 'DURRANG'}
              </h2>
              ...
              <div className="result-actions">
                <button onClick={() => startBotGame(difficulty)}>Yana o'ynash</button>
          ...
          {!opponent ? (
            ...
            <>
              ...
              {!myChoice && multiResult === null && (
                <ChoiceButtons onChoice={handleMultiChoice} />
              )}
